#!/bin/bash
#SBATCH --job-name=lstm-trai       # Job name
#SBATCH --output=training/history_lstm/lstm-%j.out      # Standard output log (%j = Job ID)
#SBATCH --error=training/history_lstm/lstm-%j.err       # Standard error log
...
python src/python_scripts/training/train_lstm.py --seq_len $SEQ_LEN $P_CONTEXT_FLAG
echo "Job finished."